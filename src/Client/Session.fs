...
let loadCart () : CartDetails option =
    let cartDecoder = Decode.Auto.generateDecoder<CartDetails> ()

    match LocalStorage.load cartDecoder CART_SESSION_KEY with
    | Ok cart -> Some cart
    | Error _ -> None

let saveCart (cart : CartDetails) = LocalStorage.save CART_SESSION_KEY cart
